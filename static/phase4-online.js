(() => {
  ...
  async function submitRun(){if(!me||!window.gameActive)return;try{await api('/api/submit-run',{method:'POST',body:JSON.stringify({score:Number(window.score)||0,seconds:Number(window.speedRunMode)?300-Number(window.speedRunRemaining||0):Number(window.seconds)||0,difficulty:window.level||'easy',mode:window.dailyMode?'daily':window.speedRunMode?'speed':'standard'})})}catch(e){console.info('Online run not submitted:',e.message)}}
  ...
  const oldFinish=window.finishGame; if(typeof oldFinish==='function'){window.finishGame=function(){const result=oldFinish.apply(this,arguments);setTimeout(()=>{if(me)submitRun()},400);return result}}
  refreshMe();
})();
