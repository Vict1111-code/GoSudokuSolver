...
func solveHandler(w http.ResponseWriter, r *http.Request) {
    ...
    for row := 0; row < 9; row++ {
        ...
        for col := 0; col < 9; col++ { ch:=line[col]; switch { case ch=='.': board[row][col]=0; case ch>='1'&&ch<='9': board[row][col]=int(ch-'0'); default: http.Error(w,"Invalid character",http.StatusBadRequest); return } }
    }
